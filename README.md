...
`before` / `after` / `blocked` / `broken`（本物のCORSエラー確認済み）の
4パターン全てで、このマシン（macOS + Hiragino搭載Chrome）では句読点の
ズレが目視再現しなかった。フォント指定の有無・CORSブロックの実態
（設定フラグ vs 本物のCORSエラー）というコード側の変数を一通り変えても
再現しないため、**再現しないのはコードの構成ではなく、このマシンに
インストール済みの日本語システムフォント（Hiragino）によるOSレベルの
フォールバックマスキングが支配的要因である**、という仮説が強く
裏付けられた。

（`MarkdownBody`経由の描画かどうか、見出し・太字・箇条書きの有無、
テーマ経由のフォント指定かどうかも同様に検証したが、いずれも同じ結果
（目視再現なし）だったため、`MarkdownBody`は句読点ズレの要因としては
考えにくいと判断し、検証コードから除外した。）

次に検討すべきは、Hiraginoのような日本語システムフォントが入っていない
環境（Linux上のheadless Chrome、Docker、CIランナーなど）でこれらの
モードを再検証すること。

## フォールバック候補の到着順レース条件の検証（issue #5 仮説2）

issue #5 で、`?only=blocked`の検証結果（`allowRuntimeFetching = false`で
`google_fonts`側のフェッチを止めてもCanvasKit自身が`notosansjp`まで独立に
取得しているのに句読点が非日本語グリフに見える、という手がかり）から、
句読点のグリフ選択は「優先言語への決定論的マッピング」ではなく、
**複数のCJKフォールバック候補（sc/hk/jp）の到着順・有無に依存するレース
条件ではないか**という仮説が提起された。

DevToolsのRequest Blockingを手動操作する代わりに、`web/index.html`で
`window.fetch`をFlutterエンジン起動前にパッチし、特定のフォントリクエスト
だけを恒久的に失敗させることで、この状況をコード側で決定論的に再現する
`?only=raceBlockJp` / `?only=raceBlockScHk`を追加した。

- **raceBlockJp**: フォント未指定（before同様）。`notosansjp`宛のフェッチ
  だけをブロックし、CanvasKitの自動フォールバックに`notosanssc`/
  `notosanshk`しか渡らない状態にする。これで句読点がズレて見えれば、
  「JP候補が存在しない」ことが直接の引き金だと確認できる。
- **raceBlockScHk**: 対照実験として逆方向（`notosanssc`/`notosanshk`を
  ブロックし`notosansjp`だけを残す）。ここで句読点が正しく見えれば、
  raceBlockJpとの差分は「JP候補の有無」が句読点のグリフ選択を左右する
  ことの裏付けになる。

ブロックが効いているかはDevTools NetworkタブでnotosansjpまたはSC/HK宛の
リクエストが`(failed) net::ERR_FAILED`になっていることで確認できる。

**検証結果**: 2モードで非対称な結果になった。

- **raceBlockJp**（notosansjpのみブロック）: 問題なく表示された。句読点の
  ズレも他の破綻も見られず、これまでの検証パターンと同様「目視では
  再現しない」という結果だった。
- **raceBlockScHk**（notosanssc/notosanshkをブロック）: **句読点どころ
  ではない破綻**が発生した。ページ全体で多数の漢字が豆腐（グリフ欠落を
  示す□）で表示され、しかもそれはサンプル文（テスト対象のテキスト）
  だけでなく、カードのラベルや説明文（テーマのデフォルトフォントで
  描画される、テスト対象ではないUIテキスト）にまで及んだ。

この非対称性は当初の仮説（sc/hkが先着すればjpの代わりにsc/hkのグリフが
選ばれる、という穏当なフォールバック置換）よりも重大な内容を示唆する。
notosansjp単体のフェッチ失敗は無害だが、notosanssc/notosanshkの
フェッチ失敗はCanvasKitのCJKフォールバック機構をページ全体で壊す
ように見える。つまりsc/hkは単なる「フォールバック候補の一つ」ではなく、
フォールバック機構が正しく機能するために必要な「代表フォント」的な
役割を担っている可能性があり、その取得失敗（本物のネットワークエラー
としての失敗）に対するエンジン側のエラーハンドリングが、単一の
フォールバック候補が使えないだけでは終わらず、他のCJKテキスト描画
全体に影響する形で壊れている可能性がある。

これは元々の「句読点の位置がズレる」症状そのものの直接的な再現とは
言えないが、issue #5 仮説2（到着順・有無に依存するレース条件）を
支持する新しい、かつより深刻な手がかりであり、CanvasKit/Flutter
エンジン側のフォントフェッチ失敗時のエラーハンドリングという、
これまで検討していなかった切り口を示している。

### 残課題: 優先言語リストの実経路（issue #5 仮説1）

issue #5 で提起されたもう一つの仮説、「`chrome://settings/languages`の
優先言語リストの実際の順序」と「JSでの`navigator.language`上書き」が
別経路である可能性については、実機のChrome設定画面を操作する必要があり
未検証。`chrome://settings/languages`で日本語を1位から外す・英語を1位に
上げる操作をした上で（JS上書きなしで）`?only=before`を目視確認する
ことが次のステップ。
